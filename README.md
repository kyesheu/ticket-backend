...
企业工单全流程管理平台后端，覆盖创建、分派、处理、关闭、评价全生命周期，集成 SLA 时效管控、动态流程引擎、全文检索和 AI 智能辅助。
...
> **项目状态：功能开发结束，进入维护阶段。** 功能路线已冻结，不再新增 v3.x 版本。
...
```bash
mvn test                                                          # Java 单元测试 (250 tests)
ai-service\.venv\Scripts\python.exe -m pytest ai-service\tests -q # Python 测试 (81 tests)
powershell scripts/ticket/v1.x/smoke-test.ps1                     # v1.x 冒烟测试
powershell scripts/ticket/v2.x/smoke-test.ps1                     # v2.x 冒烟测试
powershell scripts/ticket/v3.x/smoke-test.ps1                     # v3.x 冒烟测试
powershell scripts/ticket/v3.x/stage59-recovery-drill.ps1         # v3.3 备份恢复与故障演练
powershell scripts/ticket/v3.x/stage60-security-performance.ps1   # v3.3 安全审计与性能基线
powershell scripts/ticket/v3.x/stage61-final-release.ps1          # v3.3 全版本回归与发布门禁
powershell scripts/ticket/v3.x/stage61-final-release.ps1 -QuickCheck  # 仅文档/代码/制品审计
```
